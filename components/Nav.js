...
const Nav = () => {
  ...
  h-max bottom-0 mt-auto xl:right-[2%] z-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen '>
    ...
    gap-y-10 px-4 md:px-40 xl:px-0 h-[80px] bg-red-200/10 xl:h-max py-8 bg-white/10 backdrop-blur-sm 
    text-3xl xl:text-xl xl:rounded-full'>
      {navData.map((link, index) => {
        return <Link 
        className={`${link.path === pathname && 'text-accent'} relative flex items-center group hover:text-accent transition duration-300`}
        href={link.path} 
        key={index}
        >
          {/* Tooltip */}
          <div className='absolute pr-14 right-0 hidden xl:group-hover:flex'>
            ...
            border-y-[6px] border-r-0 absolute right-12 top-[6px]'>
            </div>
          </div>
          {/* icon */}
